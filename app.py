            ...
            if not recent_data.empty and not chart_data.empty:
                ...
                with col1:
                    ...
                    if st.button("💾 차트 HTML 다운로드"):
                        ...
                        st.info("HTML 파일을 브라우저에서 열어 스크린샷으로 저장하세요")
                
                with col2:
                    # 차트 데이터 복사 버튼
                    if st.button("📋 차트 데이터 복사"):
                        chart_summary = f"""
{ticker} 주가 정보 (최근 7일)
{'='*30}
...
거래량: {recent_data['Volume'].iloc[-1]:,.0f}

6개월 최고가: ${chart_data['High'].max():.2f}
6개월 최저가: ${chart_data['Low'].min():.2f}
"""
                        st.code(chart_summary)
                        st.success("차트 정보가 표시되었습니다. 위 텍스트를 선택하여 복사하세요!")
            else:
